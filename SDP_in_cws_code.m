    ...
    for d = 4
        ...
        for K = 9: (2^(n))
            ...
            cvx_begin sdp
                ...
                for q = 0: n
                    A_3 = find(q, 0, 0, 0, n, lst_1, lst_2);
                    B = check_num(A_3, lst);
                    if q < d
                        enumerator(x, n, K, q, lst_1, lst_2, lst)*order == order*(3^q)*comb(n, q)*x(B);
                        value = [value,enumerator(x, n, K, q, lst_1, lst_2, lst)];
                        value_1 = [value_1,(3^q)*comb(n, q)*x(B)];
                    end
                    %enumerator(x, n, K, q, lst_1, lst_2, lst)*order <= (3^q)*comb(n, q)*x(B)*order;
                    %enumerator(x, n, K, q, lst_1, lst_2, lst)*order >= 0;
                end
                clear A_3 q B;
                ...
                for p = 0: n
                    ...
                    tot_1 = cvx(zeros(1));
                    for l = 0: n
                        tot = tot + ((-1)^l)*krav(l, n, p) * enumerator(x, n, K, l, lst_1, lst_2, lst);
                        tot_1 = tot_1 + ((-1)^l)*krav(l, n, p) * enumerator(y, n, K, l, lst_1, lst_2, lst);
                    end
                    %((K^2) / (2^n))*tot*order >= 0;
                    %(1 / (2^n))*tot_1*order >= 0;
                end   
