    ...
    for d = 6: n
        for K = 1: 2^(n+c)
                ...
                cvx_begin sdp
                        ...
                        for q = 0: c
                            ...
                            for l = 0: n
                                for r = 0: c
                                    ...
                                    tot_sh = tot_sh + ((-1)^(l+r))*(K / (2^(n+c)))*krav(l, n, p) * krav(r, c, q) * x(l+1, r+1);
                                end
                            end
                            if q == 0 && p < d
                                tot == x(p+1, 1);
                    ...
                    oby == sB;
                    obt == K*(2^(n+c));
                    clear p d_1 D tot q l r z tot_1 row tot_sh;
                    if sh == 1 || mi == 1
                        enum = [];
                        for z = 0: n+c
                            tot = 0;
                    ...
                    if sh == 1
                        for p = 0: n+c
                            ...
                            if p <= n
                                tot_1 = 0;
                                for q = 0: n
                                    tot_1 = tot_1 + ((-1)^q)*krav(q, n, p)*x(q+1, 1);
                                end
                            end
                            (K / (2^(n+c)))*tot >= 0;
                            (K / (2^n))*tot_1 >= 0;
                        end
                        clear p tot l q tot_1;
                    end
